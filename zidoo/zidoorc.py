...
class ZidooRC(object):
    ...
    def load_source_list(self):
        """ Load source list from Zidoo device."""
        
        return self.get_app_list()
        
        """
        resp = self.zidoo_req_json("sony/avContent",
        ...
        if resp.get('status') == 200:
            ...
            for result in results:
                ...
                if result['source'] in ('extInput:hdmi', 'extInput:composite',
                    ...
                    resp = self.zidoo_req_json("sony/avContent", data)
                    if not resp.get('error'):
                        original_content_list.extend(resp.get('result')[0])
        
        resp = self.zidoo_req_json("sony/appControl",
    ...
    def _start_app(self, app_id, log_errors=True):
        ...
        url = 'http://{}/ZidooControlCenter/Apps/openApp?packageName={}'.format(self._host, app_id)
